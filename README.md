...
- **ディープカラー対応**: 8-bit、16-bit の両カラーフォーマットに対応
...
- **ディープカラー対応**: 8-bit、16-bit の両フォーマットに対応
- **テンプレートベース実装**: ゼロオーバーヘッドの型抽象化で両ビット深度を統一処理
...
- 両ビット深度で同一の高速アルゴリズムを使用
- `PF_WORLD_IS_DEEP` マクロによるシンプルな判定
...
| **ビット深度** | 8-bitのみ | 8-bit/16-bit対応 | 品質・互換性向上 |
